...
export const OptionCard: React.FC<OptionCardProps> = ({ option, selected, onClick, disabled }) => {
  ...
  return (
    <button 
      ...
      disabled={disabled}
      className={`
        group relative flex flex-row md:flex-col items-center md:items-start p-5 md:p-8 rounded-2xl border-[3px] w-full text-left transition-all duration-150 active:scale-[0.98]
        ${
          selected 
    ...
    >
      {Icon && (
        <div className={`
          mr-4 md:mr-0 md:mb-5 p-4 rounded-xl flex-shrink-0 transition-colors
          ${
            selected 
      ...
      <div className="flex-1">
        <span className={`text-xl md:text-2xl font-bold leading-tight block ${
          selected ? 'text-[#1da8ae]' : 'text-slate-700 group-hover:text-slate-900'
        }`}>
          {option.label}
        </span>
        {option.description && (
          <span className="text-sm md:text-base text-slate-400 font-medium mt-2 block">
            {option.description}
          </span>
        )}
      </div>
      
      <div className={`hidden md:block absolute bottom-8 right-8 transition-all duration-300 ${
        selected || 'group-hover:translate-x-1 opacity-0 group-hover:opacity-100'
      }`}>
        <ArrowRight size={24} className={selected ? 'text-[#25c9d0]' : 'text-slate-300'} strokeWidth={3} />
      </div>
    </button>
  );
};