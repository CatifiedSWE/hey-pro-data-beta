...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const calculateWorldPositions = useCallback((): WorldPosition[] => {
    ...
    for (let i = 0; i < adjustedPositions.length; i++) {
      ...
      const curSize = baseImageSize * adjustedScale;
      for (let j = 0; j < adjustedPositions.length; j++) {
        ...
        const minDistance = (curSize + otherSize) / 2 + 25;
        ...
        <div className="relative w-full h-full" style={{ zIndex: 10 }}>
          {images.map((image, index) => {
            ...
            return (
              ...
              >
                <div className="relative w-full h-full rounded-full overflow-hidden shadow-lg border-2 border-white/20 bg-gray-900">
                  <img src={image.src} alt={image.alt} className="w-full h-full object-cover" draggable={false} loading="lazy" />
                </div>
              </div>
            );
          })}
